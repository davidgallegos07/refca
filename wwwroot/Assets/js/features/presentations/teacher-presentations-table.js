$(document).ready(function () {
            ...
            success: function (data) {
                ...
                $.each(data.items, function (key, value) {
                    datax +=
                        `<tr>
                    <td>${value.title}</td>
                    <td>${value.congress}</td>
                    <td>${value.editionDate}</td>
                    <td class="id" hidden>${value.id}</td>
                    <td class="path" hidden>${value.presentationPath}</td>  
                    <td>`;
                    $.each(value.teacherPresentations, function (key, value) {
    ...
    $(tbody).on('click', 'tr', function () {
        ...
        $.ajax({
            url: `/api/teacher/presentations/${id}/role`,
            success: function (role) {
                ...
                console.log(path);
                data = `
                <li class="${authorized}"><a href="/Presentation/Edit/${id}">Editar</a></li>
                <li class="${authorized}"><a href="/Presentation/Upload/${id}">Añadir PDF</a></li>
                <li class="${existPath}"><a target="_blank" href="${path}">Descargar</a></li>
                    <li class="${authorized}">
                        <form action="/Presentation/Delete/${id}" method="post" class="js-delete">
